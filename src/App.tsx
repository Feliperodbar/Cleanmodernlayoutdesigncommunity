...
import {
  ...
  updatePersistedCustomer,
  addPersistedCustomer,
} from "./data/db";
...
export default function App() {
  const [currentPage, setCurrentPage] = useState<
    ...
    | "service"
    | "register"
    | "new_connection_checklist"
    | "new_connection_address"
    | "new_connection_installation"
    | "new_connection_equipment"
    | "new_connection_summary"
    | "new_connection_services"
  >("search");
  const [selectedCustomer, setSelectedCustomer] = useState<Customer | null>(
    null
  );
  const [registerInitialDocument, setRegisterInitialDocument] = useState<
    string | undefined
  >(undefined);
  const [newConnectionAddressData, setNewConnectionAddressData] =
    useState<NewConnectionAddressData | null>(null);
  const [newConnectionInstallationData, setNewConnectionInstallationData] =
    useState<NewConnectionInstallationData | null>(null);
  const [newConnectionEquipmentData, setNewConnectionEquipmentData] =
    useState<NewConnectionEquipmentData | null>(null);
  const [
    newConnectionSelectedDistributor,
    setNewConnectionSelectedDistributor,
  ] = useState<string | null>(null);
  const [pendingAction, setPendingAction] = useState<
    "new_connection" | "second_invoice" | "outage" | "update" | null
  >(null);
  ...
  const createNewUCAddress = (): Address | null => {
    ...
    const line = `${newConnectionAddressData.address}${
      newConnectionAddressData.number
        ? ", " + newConnectionAddressData.number
        : ""
    }${
      newConnectionAddressData.neighborhood
        ? ", " + newConnectionAddressData.neighborhood
        : ""
    }`;
    return {
  ...
  if (currentPage === "search") {
    return (
      <SearchPage
        ...
        onQuickNewConnection={(dist) => {
          setNewConnectionSelectedDistributor(dist);
          setPendingAction("new_connection");
        }}
        onQuickSecondBill={() => {
          setPendingAction("second_invoice");
        }}
        onQuickOutage={() => {
          setPendingAction("outage");
        }}
        onQuickUpdate={() => {
  ...
  if (currentPage === "new_connection_address") {
    return (
      <NewConnectionAddressPage
        ...
        onNext={(data) => {
          setNewConnectionAddressData(data);
          setCurrentPage("new_connection_installation");
        }}
  ...
  if (currentPage === "new_connection_installation") {
    return (
      <NewConnectionInstallationPage
        ...
        onNext={(data) => {
          setNewConnectionInstallationData(data);
          setCurrentPage("new_connection_equipment");
        }}
        onCancel={() => setCurrentPage("service")}
        customer={selectedCustomer}
      />
  ...
  if (currentPage === "new_connection_equipment") {
    return (
      <NewConnectionEquipmentPage
        ...
        onNext={(data) => {
          setNewConnectionEquipmentData(data);
          setCurrentPage("new_connection_summary");
        }}
        onCancel={() => setCurrentPage("service")}
        customer={selectedCustomer}
      />
      ...
      <NewConnectionServicesPage
        ...
        onConfirm={() => {
          ...
          if (addr && selectedCustomer) {
            ...
            if (exists) {
              const updated = updatePersistedCustomer(selectedCustomer.id, {
                addresses: [addr, ...exists.addresses],
              });
              if (updated) {
